...
BASE_URL = 'http://tkmenus.com/tankandpaddle/'
...
# Outputs
path_out = create_folder('68_TankPaddle', folder)
file_json = os.path.join(path_out, 'tankpaddle_items.json')
file_csv = os.path.join(path_out, 'tankpaddle_items.csv')
...
def parse_page(html: str) -> List[Dict]:
    ...
    for item in items:
        ...
        labels_container = item.select_one('div.k10-recipe__labels-wrapper-content')
        allergen_labels = [s.strip() for s in labels_container.stripped_strings] if labels_container else []
