# MenuTracker Copilot Instructions
...
**Chrome Driver Management**: The `setup_driver()` function in helpers.py attempts `undetected_chromedriver` first (better evasion), falls back to `webdriver-manager` if needed. Detects local Chrome version automatically and handles headless configuration.
...
**Chrome Driver**: Download compatible ChromeDriver if `undetected_chromedriver` fails. The code auto-manages webdriver via `webdriver-manager`, but manual setup may be needed for Colab.
...
- **Chrome Driver Fails**: Check Chrome is installed; if `undetected_chromedriver` hangs, it will auto-fall back to webdriver-manager version
